...
class ValidateWebSiteCommand extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if(!$input->getOption('only-errors')) $errors = 0;
        if($input->getOption('concurrent-requests')) $concurrent = $input->getOption('concurrent-requests');
        ...
        for ($i=0; $i < count($res); $i++) { 
            ...
            if($tags === "") $table[$i]["tags"] = "empty tags";
            else $table[$i]["tags"] = $tags;
        }
